...
def apply_for_job():
    ...
    cursor.execute(f"SELECT skill_id from skill where academic_qualifications='{j_academic_qualifications.get()}' AND cocurricular='{j_cocurricular_activities.get()}' ")
...
def job_window():
    ...
    global j_post,j_academic_qualifications,j_cocurricular_activities,j_package,j_criteria,j_emp_name,j_information
    j_post = StringVar()
    ...
    technical_text = scrolledtext.ScrolledText(DataFrameLEFT, undo=True,width=48,height=1,font=('arial',15,'bold'))
    technical_text.grid(row=1,column=1)
...
def login_employer():
    ...
    root_employer.title("Log-In Employer")
...
if __name__ == "__main__":
    ...
    heading = Label(root,text="Welcome to Online Recruitment System!",font=("arial", 14,"bold underline"),bg='light grey')
    welcome = Label(root,text="Hello User, we welcome you here",font=('Courier',10,"italic bold"),bg='light grey')
    heading.grid(row=0,column=1,padx=70)
    welcome.grid(row=1,column=1,padx=70)

    LogIn = Menubutton(root, text="Log-In", fg="Black", bg="white",activebackground='black',activeforeground='white',bd=3,relief=SUNKEN,width=12) 
    Register = Menubutton(root, text="Register", fg="Black", bg="white",activebackground='black',activeforeground='white',bd=3,relief=SUNKEN,width=12) 
